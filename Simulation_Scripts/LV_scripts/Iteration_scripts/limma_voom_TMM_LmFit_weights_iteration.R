...
#' @param norm_method Normalization method to use (default: "TMM")
...
analyze_null_simulation_batch <- function(data_dir,
  ...
  for (i in seq_along(all_files)) {
    ...
    # Run analysis
    result <- run_limma_voomlmFit_DE0(
    ...
    if (is.na(result$n_genes_tested)) {
      n_failed <- n_failed + 1
    } else {
  ...
  #### Step 7: Save results ####
  output_filename <- sprintf("%s_DE0_edgeR_%s_summary.csv", dataset_name, norm_method)
...
bc_results <- analyze_null_simulation_batch(
  ...
  norm_method = "TMM" # works fine with TMMwsp as well
)
